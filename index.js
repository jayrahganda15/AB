...
app.listen(3000, () => {
		console.log(`
\x1b[34mAstralAI - Kaito is online\x1b[0m
...
async function accountLogin(state, enableCommands = [], prefix, admin = []) {
						...
						try {
								var listenEmitter = api.listenMqtt(async (error, event) => {
										...
										if (hasPrefix && aliases(command)?.hasPrefix === false) {
												api.sendMessage(`❎️ | Invalid usage this command doesn't need a prefix`, event.threadID, event.messageID);
												return;
										}
										if (event.body && aliases(command)?.name) {
												...
												if ((role == 1 && !isAdmin) || (role == 2 && !isThreadAdmin) || (role == 3 && !config?.[0]?.masterKey?.admin?.includes(event.senderID))) {
														api.sendMessage(`🚯 | You don't have permission to use this command. only Developer && @KaitoDev (Fritz Martin Amparado)`, event.threadID, event.messageID);
														return;
												}
										}
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == userid)) {
    api.changeNickname(`[ ${prefix} ] - BOT`, threadID, userid);
												
		let gifUrl = 'https://i.imgur.com/8XH3kg8.gif';
